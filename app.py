...
def _load_genome_info() -> dict[str, dict[str, str]]:
    """Load genome.information.xlsx → {abbr: {full_name, assembly, url}}.

    The file has a single header row (row 1):
      A = full name (Row name), B = abbreviation, C = assembly, D = URL
    """
    ...
    try:
        ...
        for row in rows[1:]:  # skip header row
            ...
            for c in row.findall("a:c", XLSX_NS):
                letters = _col_letters(c.attrib.get("r", ""))
                if letters:
                    by_col[letters] = _cv(c)
            full_name = (by_col.get("A") or "").strip()
            abbr = (by_col.get("B") or "").strip()
            assembly = (by_col.get("C") or "").strip().rstrip("\t")
            url = (by_col.get("D") or "").strip()
            if abbr:
                result[abbr] = {
                    "full_name": full_name or abbr,
                    "assembly": assembly,
                    "url": url,
                }
        return result
    ...
    try:
        ...
        if chosen:
            ...
            if picked:
                ...
                # Preserve order, drop duplicate assay names.
                seen_assays: set[str] = set()
                assay_parts: list[str] = []
                for a in assays:
                    if a in seen_assays:
                        continue
                    seen_assays.add(a)
                    assay_parts.append(a)
                label = (
                    f"{gid} · {' / '.join(assay_parts)}" if assay_parts else gid
                )
                mo_item = {
                    "label": label,
                    "href": (
...
def api_genome_info():
    """Return genome info mapping: abbr → {full_name, assembly, url}."""
...
def api_genome_region_gtf_export():
    ...
    try:
        if transcript_id:
            ...
            # Region UI queries stay coordinate-sorted; export restores
            # original GTF file order via source_line.
...
def api_download_generate():
    """Retired matplotlib download path — use /api/pygt/submit instead."""
    abort(
        410,
        description=(
            "The matplotlib download engine has been retired. "
            "Please use the pyGenomeTracks download flow."
        ),
    )
...
#
# Download visualization is rendered via pyGenomeTracks. Jobs run as a
# subprocess and can take several seconds, so they are queued on a small
...
def api_pygt_result(job_id: str):
    """Fetch the rendered image artifact. .ini / run.log are not exposed."""
    if not re.fullmatch(r"[0-9a-f]{8,32}", job_id):
        abort(400, description="invalid job_id")
    kind = (request.args.get("kind") or "image").lower()
    # Do not serve ini/log — they may contain internal paths / config details.
    if kind in ("ini", "log"):
        abort(404, description="not available")
    if kind != "image":
        abort(400, description="kind must be image")
