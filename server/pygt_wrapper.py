...
Extra INI properties:
    ...
    row_scale_factor – multiplier for inter-row spacing
                     (default: 2.3, pyGenomeTracks built-in). Use ~1.2–1.3 for
                     denser Homologous Seq/Loci stacks.
...
def _patched_draw_arrow(self, start, end, strand, ypos):
    ...
    Tip length prefers ``small_relative * arrow_scale`` so all normal-length
    features in one plot share the same tip width; only features shorter than
    ``2 * desired_tip`` fall back to half their span.
    """
    half_height = float(self.properties['interval_height']) / 2
    scale = float(self.properties.get('arrow_scale', 1.0) or 1.0)
    span = max(0.0, float(end) - float(start))
    desired = float(self.small_relative) * scale
    # Same tip for every feature that is at least 2× desired tip long.
    tip = desired if span >= 2.0 * desired else span * 0.5
...
def _patched_bed_init(self, *args, **kwargs):
    """BedTrack.__init__ then honour optional ``row_scale_factor`` from INI."""
    _patched_bed_init._orig(self, *args, **kwargs)
    try:
        factor = float(self.properties.get('row_scale_factor', _DEFAULT_ROW_SCALE_FACTOR)
                       or _DEFAULT_ROW_SCALE_FACTOR)
    except (TypeError, ValueError):
        factor = _DEFAULT_ROW_SCALE_FACTOR
    factor = max(1.0, min(5.0, factor))
    try:
        interval_h = float(self.properties['interval_height'])
    except (TypeError, ValueError, KeyError):
        interval_h = 100.0
    self.row_scale = interval_h * factor


def install_patches() -> None:
    from pygenometracks.tracks import BedTrack as _BedTrackClass
    if not getattr(_BedTrackClass.__init__, '_perv_patched', False):
        _patched_bed_init._orig = _BedTrackClass.__init__
        _patched_bed_init._perv_patched = True
        _BedTrackClass.__init__ = _patched_bed_init
